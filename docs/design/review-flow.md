...
- **With `"viewNotesInside": true`, `o` shows the note in the review window instead** (#51): read-only, rendered, scrolled to the card's line and highlighted. The viewer is a session state (`Session.viewer`), and while it shows only `host`'s `interpretViewingKey` is consulted — `o` and `Escape` go back to the card as it was, `e` opens the note in the configured `editor` through the ordinary `o` path, and `1`–`4`, `q`, `0` and `a` do nothing. `openIn` is fixed when the sitting's queue is drawn. The card is found in the note by its stamp, not its stored line, since the note is read from disk and may have changed since the sync. How the note is rendered and sanitised is in [app.md](app.md#a-note-is-user-content-the-viewer).
...
- **A card on a learning step comes back in the same session.** Our pinned parameters have `enable_short_term` on with `learning_steps: ["1m", "10m"]` and `relearning_steps: ["10m"]`, so a new card rated `1` is due in 1 minute, `2` in 6, `3` in 10, and only `4` graduates; a Review card rated `1` goes to Relearning 10 minutes out. Those due dates are honoured rather than discarded — see [the queue](#the-queue) below and [ADR 0023](../decisions/0023-honour-short-term-learning-steps.md). The figures are FSRS-6's ([ADR 0028](../decisions/0028-move-to-fsrs-6.md)); ADR 0023's table records FSRS-5's.
