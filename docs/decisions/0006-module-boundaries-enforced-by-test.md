...
- **Source:** [phase-1 brief](../design/phase-1-brief.md) §6
...
- Injecting the clock and the ID generator is not purity for its own sake: without them the tests in the [phase-1 brief](../design/phase-1-brief.md) §10 cannot be written at all. Asserting that stamping is idempotent needs predictable IDs, and asserting that a card comes due needs to fast-forward past its interval.
